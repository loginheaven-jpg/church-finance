...
3. **각 반영 단계 크로스체크 추가**
   ...
   - 수입부 반영: pending 입금 0건 확인
   - 지출부 반영: pending 출금 0건 확인
...
| 항목 | 내용 | 우선순위 |
|------|------|---------|
| Supabase 직접 업로드 | 영수증 모바일 업로드 시 Vercel 4.5MB 제한 우회. 현재 클라이언트 이미지 압축(800px JPEG 80%)으로 임시 해결. 다건 첨부 시 합산 초과 가능성 잔존 | 낮음 |
| 카드대금 자동 감지 강화 | match/auto에서 카드 키워드 감지 시 needsReview 분류 (현재는 일반 지출로 matched) | 중간 |
