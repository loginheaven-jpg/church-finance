...
| 메뉴 | 경로 | 최소 권한 | 비고 |
|------|------|----------|------|
| 지출청구 | `/expense-claim` | **member** | 2탭: 청구 입력, 청구 현황 (지출부 대조 통합) |
| 데이터 입력 | `/data-entry` | admin | 4탭: 현금헌금 동기화, 은행원장 입력, 미처리 확인, 수입부 데이터보정 |
| 거래 매칭 | `/match` | **super_admin** | 비상용 (정상 운영 시 미사용) |
...
**수정 (A·B·C):** verification 을 2단계로 재작성.
- **전역 1:1 배정(A)** — (임계65 && 이름일치) 후보쌍을 점수 내림차순 greedy 로 배정하되 **지출부 레코드는 청구 1건에만 소비**. greedy 후 미배정 청구를 미사용 지출부에 재시도하는 2차 패스로 최대매칭 근사.
- **내역 가중 상향(B)** 10→15, **날짜 근접(C)** 을 지출부 실제 거래일(transaction_date)·청구일/처리일 최근접 **연속 함수(20−일수)** 로 계산 → ±7일 내부에서도 구분력 확보. pairs 정렬 tie-break: 점수 → dd(날짜근접) → 청구인덱스로 **결정성** 보장.
- 미배정 사유는 "임계+이름일치 후보가 있었으나 다른 청구에 선점됨(중복 상신 여부 확인)"으로 표기(`cands.some`). 잘못된 전역 조회범위(비정상 날짜) 방어 가드 추가.
- **검증(실데이터):** 8/3 → 최종확인(score 85), 8/6 → 입금완료(score 82, 중복 상신 의심). 결정적.
- '최종확인'은 저장값이 아니라 대조 계산 결과라 **시트 데이터 수정 불필요** — 배포 후 "처리내역 점검"이 돌면 8/6이 자동으로 입금완료로 표시됨.
...
1. **기존 흐름 (금액 먼저 → AI 사후검증)** — 유지
   - 사람이 금액 입력 → 영수증 첨부 → AI 인식금액과 비교 → 불일치 시 사유 입력 팝업(`aiPopup`)

2. **신규 흐름 (영수증 먼저 → 금액 자동입력)** — 추가
   - 금액칸이 **비어 있을 때** 영수증 첨부 → AI 분석 완료 후 자동입력 팝업(`prefillPopup`) → 인식금액을 그대로 쓰거나 수정해 확정 → 금액칸 자동 채움
   - 판정 조건 `canPrefill`: 금액이 비었거나, 이전에 AI가 자동입력한 값(`autoFilled`)을 사용자가 그대로 둔 경우(복수 영수증 추가 시 합계로 재제안)
   - 인식금액을 그대로 사용하면 `autoFilled=true` → 제출 시 내역 태그 `(※AI자동입력)`. 팝업에서 금액을 수정하면 `mismatch`로 떨어져 기존 사유입력 흐름으로 연결
   - 오인식/미인식 시 팝업의 "직접 입력" 또는 실패(`failed`) 처리로 수동입력 자연 폴백
   - 금액을 사람이 직접 수정하면 `autoFilled=false`로 해제(`recheckAI`)
...
1. **지출청구 복수 영수증 첨부**
   - 항목당 1개 → 복수 파일 첨부 (multiple 선택 + 추가 첨부)
   - receiptFile: File | null → receiptFiles: File[], receiptPaths: string[]
   - 개별 파일 삭제(X) 버튼, 그룹핑 시 모든 경로 합산

2. **3/22 주차 시뮬레이션 완료 — 정합성 검증 통과**
   - 은행원장 56건 = XLS ✅, 입금/출금 합계 ✅
   - 헌금함: suppressed 945,000 = 수입부 헌금함 945,000 ✅
   - 수입: matched 3,916,050 = 계좌이체 3,916,050 ✅
   - 지출: matched 7,734,830 = 은행이체 7,734,830 ✅
   - 누적잔액: 86,870,071 = XLS 마감잔고 ✅

### 2026-03-26 업데이트 (데이터 정합성 강화 + 카드내역 개선)

1. **match/confirm 반영 순서 변경** (치명적 버그 수정)
   - 기존: 은행원장 status 먼저 변경 → 수입/지출부 쓰기 (실패 시 복구 불가, 영원히 pending 잔류)
   - 수정: 수입/지출부 먼저 쓰기 → 성공 시에만 은행원장 status 변경
   - updateBankTransactionsBatch: 별도 readSheet 제거 → confirm에서 전달받은 cachedRows 사용

2. **Google Sheets API 호출 최적화**
   - 은행원장 readSheet 1회만 실행, cachedRows로 재사용 (~80회 → ~5회)
   - incrementRuleUsage 개별 호출 제거 (API 할당량 보호)

3. **각 반영 단계 크로스체크 추가**
   - 현금헌금 동기화: 수입부 헌금함 합계 = 소스 합계 확인
   - 은행원장 업로드: 시트 건수/입금/출금 = 업로드 데이터 확인
   - 수입부 반영: matched입금 = 수입부 계좌이체, suppressed = 헌금함 확인
   - 지출부 반영: matched출금 = 지출부 은행이체, pending = 0건 확인
   - 카드내역 반영: 법인카드 합계 + NH카드대금 삭제 + 은행원장 suppressed 확인
   - 모든 결과 toast 팝업으로 즉시 안내

4. **카드내역입력 개선**
   - 반영(apply) 시 은행원장 카드대금 출금 자동 suppressed 처리
   - 반영 후 applied 건 보존 표시 (녹색 배경 + "반영완료" 라벨, 읽기전용)
   - 새 카드파일 업로드 시까지 이전 반영 결과 확인 가능

5. **은행원장 입력 통합**
   - 매칭 후 수입부/지출부 반영 버튼을 같은 화면에 배치
   - 미반영처리 탭 → "미처리 확인" 이름 변경 (pending 0건이면 ✅ 표시)

6. **교차검증 잔액 계산 수정**
   - 자본이동(40번대 수입, 92/93 지출) 제외하던 것을 전체 합산으로 변경
   - 은행 잔액과 비교 시 정확한 잔액 표시

7. **로그인 화면 SSO 일원화**
   - "재정시스템 로그인" + "교적부에서 로그인하기" → "예봄성도 로그인" 통합
   - 모바일 로그인 화면에서 사이드바 제거
   - 인증 후 원래 요청 URL로 복귀 (redirect 파라미터 전달)

8. **페이지별 OG 메타데이터**
   - 카톡 공유 시 "재정부 > 지출청구" 등 페이지명 표시

9. **거래매칭 메뉴 super_admin 전용**
   - 정상 운영 시 사용 불필요 (데이터입력에서 매칭+반영 통합)
   - 비상 시(과거 데이터 복구 등) super_admin만 접근

## 향후 작업 검토

| 항목 | 내용 | 우선순위 |
|------|------|---------|
| Supabase 직접 업로드 | 영수증 모바일 업로드 시 Vercel 4.5MB 제한 우회. 현재 클라이언트 이미지 압축(800px JPEG 80%) + Supabase signed URL로 직접 업로드 구현됨. 대부분 300~400KB로 압축되어 실용적 문제 해소. 다건 첨부 시 합산 초과 가능성 잔존 | 낮음 |
| 카드대금 자동 감지 강화 | match/auto에서 카드 키워드 감지 시 needsReview 분류. 현재는 카드내역 apply 시 수동 suppressed 처리됨 | 낮음 |
