...
모든 레코드 ID는 다음 형식으로 생성됩니다:

```
{PREFIX}{YYYYMMDD}{timestamp6}{random4}
```

| 구성요소 | 설명 | 예시 |
|---------|-----|-----|
| PREFIX | 레코드 유형 | `INC`(수입), `EXP`(지출), `BANK`(은행거래), `RULE`(규칙) |
| YYYYMMDD | 생성일 (KST) | `20260120` |
| timestamp6 | 타임스탬프 끝 6자리 | `527214` |
| random4 | 랜덤 4자리 (a-z, 0-9) | `x7k2` |

**예시:** `EXP20260120527214x7k2`
