            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={recentChartData} barGap={4} barCategoryGap="15%" margin={{ top: 30, right: 10, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey="건축헌금" stackId="income" fill="#22c55e" name="건축헌금">
                  <LabelList dataKey="건축헌금" position="center" formatter={(v) => { const n = Number(v); return n > 0.3 ? `${Math.round(n * 100)}` : ''; }} style={{ fill: '#fff', fontSize: 14, fontWeight: 'bold' }} />
                </Bar>
                <Bar dataKey="일반예산" stackId="income" fill="#166534" name="일반예산">
                  <LabelList dataKey="일반예산" position="center" formatter={(v) => { const n = Number(v); return n > 0.3 ? `${Math.round(n * 100)}` : ''; }} style={{ fill: '#fff', fontSize: 14, fontWeight: 'bold' }} />
                ...
                <Bar dataKey="원금상환" stackId="expense" fill="#f97316" name="원금상환">
                  <LabelList dataKey="원금상환" position="center" formatter={(v) => { const n = Number(v); return n > 0.3 ? `${Math.round(n * 100)}` : ''; }} style={{ fill: '#fff', fontSize: 14, fontWeight: 'bold' }} />
                </Bar>
                <Bar dataKey="이자지출" stackId="expense" fill="#b91c1c" name="이자지출">
                  <LabelList dataKey="이자지출" position="center" formatter={(v) => { const n = Number(v); return n > 0.3 ? `${Math.round(n * 100)}` : ''; }} style={{ fill: '#fff', fontSize: 14, fontWeight: 'bold' }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            </div>
