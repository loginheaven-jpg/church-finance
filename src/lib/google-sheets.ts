...
export async function autoTransferBankToLedger(
  ...
  // P1 (2026-08): 원장 자연키 중복스킵(C, β⁶) 제거.
  //   같은 날·같은 금액·같은 적요(detail)의 "서로 다른 실지급"(예: 학비지원 이민우 3M + 방민혁 3M,
  //   적요가 모두 "11학비지원")을 재유입 중복으로 오판 → 두 번째 지출부 미기록(누락) 유발.
  //   재업로드 중복은 A(업로드 중복키 = date+time+금액+detail, time 포함)가 이미 원천 차단하고,
  //   같은 은행 tx 재이관은 아래 [bank:id] 마커 스킵이 막으므로 C는 불필요. 오탐만 남아 제거함.
